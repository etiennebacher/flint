...
`lint()` returns a dataframe containing all the lines with a non-idiomatic or
unconventional code.
...
`lintr` cannot perform automatic replacement of lints. This task is usually 
delegated to `styler`.
...
`sgconfig.yml` and a folder `rules` (both of these will be put in `.Rbuildignore`
but not in `.gitignore`).

Each file in the `rules` folder contains the definition of a lint and (in most
cases) a fix for this lint.

## Providing custom rules

You can add your own custom rules in the folder `rules/custom`. In this folder,
you can them organize them as you prefer (e.g creating other subfolders).

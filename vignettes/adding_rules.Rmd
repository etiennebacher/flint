...
`tinylint` comes with an extensive set of rules taken from `lintr`, but if 
...
parameter in `ast-grep`. By default, this is `smart` but we want to ignore quotation
so we set it to `ast` (see [here](https://ast-grep.github.io/advanced/match-algorithm.html#strictness-examples) for examples):
